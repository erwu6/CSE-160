function hummingbird(offsetX = 0, offsetY = 0, offsetZ = 0){
  ...
  // choose texture mode once based on the normal toggle
  var tex = g_normalOn ? -3 : -2;

  function lighting(base, normal){
    var nDotL = Math.max(0,normal.dot(g_lightDir));
    var bright = Math.pow(max(nDotL, 1.5));
    return[base[0]*bright,base[1]*bright, base[2]*bright,base[3] || 1.0];
  }

  function isMirrored(matrix) {
    // matrix.elements is a Float32Array of 16 elements (column-major)
    // The diagonal entries (0,5,10) correspond roughly to scaleX, scaleY, scaleZ
    let scaleX = matrix.elements[0];
    let scaleY = matrix.elements[5];
    let scaleZ = matrix.elements[10];
    let detSign = scaleX * scaleY * scaleZ;
    return detSign < 0; // true if mirrored
}

  function updateNormals(x){
    if (!x) return;

    // If single object, wrap it into an array
    if (!Array.isArray(x)) x = [x];

    for (let i of x){
        // compute normalMatrix as usual
        i.normalMatrix.setInverseOf(i.matrix).transpose();

        // if mirrored, flip normalMatrix to correct lighting
        if (isMirrored(i.matrix)){
            i.normalMatrix.scale(-1, -1, -1);
        }
    }
}
//draw a body
  ...
  tailFan1.normalMatrix.setInverseOf(tailFan1.matrix).transpose();
  updateNormals(tailFan1);
  tailFan1.render();

  var tailFan2 = new TriPrism();
  tailFan2.color = [10/255, 153/255 + g_yellowAngle/600, 136/255 + g_yellowAngle/300,1.0];
  tailFan2.textureNum = tex;
  ...
  tailFan2.normalMatrix.setInverseOf(tailFan2.matrix).transpose();
  updateNormals(tailFan2);
  tailFan2.render();

  var tailFan3 = new TriPrism();
  tailFan3.color = [10/255, 153/255 + g_yellowAngle/600, 136/255 + g_yellowAngle/300,1.0];
  tailFan3.textureNum = tex;
  ...
  tailFan3.normalMatrix.setInverseOf(tailFan3.matrix).transpose();
  updateNormals(tailFan3);
  tailFan3.render();
  ...
  smallerRightWing.textureNum = tex;
  ...
  bigLefttWing.normalMatrix.setInverseOf(bigLefttWing.matrix).transpose();
    updateNormals(bigLefttWing);
  bigLefttWing.render();

  var smallLeftWing = new TriPrism();
  smallLeftWing.color = [10/255, 153/255 + g_yellowAngle/100, 136/255 + g_yellowAngle/300,1.0];
  smallLeftWing.textureNum = tex;
  ...
  smallerLeftWing.textureNum = tex;
  ...
  updateNormals(rightLeg);
  rightLeg.render();

  var rightFoot = new Cube();
  rightFoot.color = [9/255, 77/255, 23/255, 1];
  rightFoot.textureNum = tex;
  ...
  updateNormals(rightFoot);
  rightFoot.render();

  var rightTalon1 = new Cube();
  // 9, 77, 23
  rightTalon1.color = [0, 0,0, 1];
  rightTalon1.textureNum = tex;
  ...
  rightTalon1.normalMatrix.setInverseOf(rightTalon1.matrix).transpose();
  updateNormals(rightTalon1);
  rightTalon1.render();

  var rightTalon2 = new Cube();
  rightTalon2.color = [0, 0,0, 1];
  rightTalon2.textureNum = tex;
  ...
  rightTalon2.normalMatrix.setInverseOf(rightTalon2.matrix).transpose();
  updateNormals(rightTalon2);
  rightTalon2.render();

  var rightTalon3 = new Cube();
  rightTalon3.color = [0, 0,0, 1];
  rightTalon3.textureNum = tex;
  rightTalon3.matrix = new Matrix4(rightTalon1.matrix);
  rightTalon3.matrix.translate(0, 0, 1.3);
  rightTalon3.normalMatrix.setInverseOf(rightTalon3.matrix).transpose();
  updateNormals(rightTalon3);
  rightTalon3.render();

  var leftLeg = new Cube();
  leftLeg.color = [9/255, 77/255, 23/255, 1];
  leftLeg.textureNum = tex;
  ...
  updateNormals(leftFoot);
  leftFoot.render();

  var leftTalon1 = new Cube();
  leftTalon1.color = [0,0,0,1];
  leftTalon1.textureNum = tex;
  ...
  leftTalon1.normalMatrix.setInverseOf(leftTalon1.matrix).transpose();
  updateNormals(leftTalon1);
  leftTalon1.render();

  var leftTalon2 = new Cube();
  leftTalon2.color = [0,0,0,1];
  leftTalon2.textureNum = tex;
  leftTalon2.matrix = new Matrix4(leftTalon1.matrix);
  leftTalon2.matrix.translate(0, 0, 1.3);
  leftTalon2.normalMatrix.setInverseOf(leftTalon2.matrix).transpose();
  updateNormals(leftTalon2);
  leftTalon2.render();

  var leftTalon3 = new Cube();
  leftTalon3.color = [0,0,0,1];
  leftTalon3.textureNum = tex;
  leftTalon3.matrix = new Matrix4(leftTalon1.matrix);
  leftTalon3.matrix.translate(0, 0, 1.3);
  leftTalon3.normalMatrix.setInverseOf(leftTalon3.matrix).transpose();
  updateNormals(leftTalon3);
  leftTalon3.render();
}